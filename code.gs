...
const CATEGORY_GROUPS = [
  ...
  //   '房租'                    — 最早期历史数据（无后缀），保留兼容
  //   '房租(浦安)'              — 中期历史数据，保留兼容
  //   '房租(浦安・銀行引落)'    — 现行值（浦安 predebit_rent，银行引落直接扣）
  //   '房租(エポス代扣・横浜)'  — 现行值（横浜 predebit_rent_epos，管理公司指定エポス卡代扣）
  //   '房租(横浜和田町)'        — 最早期历史数据，保留兼容
  { label: '房租',           cats: ['房租', '房租(浦安)', '房租(浦安・銀行引落)', '房租(エポス代扣・横浜)', '房租(横浜和田町)'] },
